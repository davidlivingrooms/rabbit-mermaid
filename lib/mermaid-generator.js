...
class MermaidGenerator {
  ...
  isRelevantExToExBinding(binding, exchangesOpt) {
    const isSourceRelevant =
      exchangesOpt.find(exOpt => binding.source === exOpt) !== undefined;
    console.log("isSourceRelevant", isSourceRelevant);
    const isDestinationRelevant =
      exchangesOpt.find(exOpt => binding.destination === exOpt) !== undefined;
    console.log("isDest", isDestinationRelevant);
    return isSourceRelevant && isDestinationRelevant;
  }

  genExchangeToExchangeBlocks(json, opts) {
    const exBuffer = [];
    const allExToExBindings = this.findExToExBindings(json);
    let relevantExchanges;
    if (opts.exchanges.length === 0) {
      // No exchanges option passed in. Everything gets processed
      relevantExchanges = allExToExBindings;
    } else {
      console.log("options", opts);
      relevantExchanges = allExToExBindings.filter(binding =>
        this.isRelevantExToExBinding(binding, opts.exchanges)
      );
    }

    relevantExchanges.forEach(binding => {
  ...
  generate(opts = {}) {
    ...
    const exToExText = this.genExchangeToExchangeBlocks(this.json, options);
    if (exToExText) {
