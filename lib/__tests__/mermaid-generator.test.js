...
describe("MermaidGenerator", () => {
  ...
  it("throws if an unrecognized ex is passed into getExchangeBlock", () => {
    const unknown = unknownExchange.exchanges.find(ex => ex.name === "breaker");
    const mermaidGenerator = new MermaidGenerator(unknownExchange);
    expect(() => {
      mermaidGenerator.genExchangeBlock(unknown);
    }).toThrow();
  });
...
PNG --> GIF`;
    ...
    const opts = {
      showExToExBindings: true
    };

    assert.deepEqual(mermaidGenerator.generate(opts), expectedString);
  });

  it("handles topology files that do not contain exchange to exchange bindings when the showExToExBindings option is set", () => {
    const mermaidGenerator = new MermaidGenerator(multiple, {});
    const expectedString = `graph LR
subgraph JPEG
JPEG(("JPEG<br/>(direct)"))
...
GIF --routing_key=*.resize--> gresizer("gresizer")
  ...
  it("ignores exchange to exchange bindings if one of them have been filtered out", () => {
    const mermaidGenerator = new MermaidGenerator(exchangeToExhange, {});
    const expectedString = `graph LR
subgraph JPEG
JPEG(("JPEG<br/>(direct)"))
